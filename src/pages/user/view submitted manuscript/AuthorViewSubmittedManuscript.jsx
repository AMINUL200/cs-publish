  ...
  const getManuscript = async () => {
    try {
      const response = await axios.get(
        `${API_URL}api/author/manuscript-update-status`,
        {
          headers: {
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <Link
                    to={`/confirmation/view-manuscript/${item.id}`}
                    className="flex-1 group"
                  >
                    <h3
                      className="text-lg font-medium group-hover:text-blue-500 "
                      dangerouslySetInnerHTML={renderHtml(
                        item?.title || "Untitled",
                      )}
                    />
                    <p
                      className="mt-1 text-sm text-gray-600 line-clamp-3 group-hover:underline "
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <span
                      className={`text-xs px-2 py-1 rounded-full ${item?.manuscript_status === "Published" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                    >
                      {item?.manuscript_status || "Unknown"}
                    </span>
                    {needsUpdate ? (
                      <button
                        className="text-xs px-3 py-1 rounded bg-red-600 text-white hover:bg-red-700 cursor-pointer"
                ...
                {Array.isArray(item?.assignmen) &&
                  item.assignmen.length > 0 && (
                    <div className="mt-4 border-t border-gray-100 pt-3">
                      ...
                      <div className="flex flex-col gap-3">
                        {item.assignmen.map((asgn) => {
                          const hasMessages =
                            Array.isArray(asgn?.messages) &&
                            asgn.messages.length > 0;
                          return (
                            <div
                              key={asgn.id}
                            ...
                            >
                              <div className="flex flex-wrap items-center gap-2 text-sm">
                                ...
                                <span className="px-2 py-0.5 rounded bg-blue-50 text-blue-700">
                                  Reviewer: {asgn.assigned_to}
                                </span>
                                <span className="px-2 py-0.5 rounded bg-purple-50 text-purple-700 capitalize">
                                  {asgn.status}
                                </span>
                                <span className="px-2 py-0.5 rounded bg-amber-50 text-amber-700 capitalize">
                                  {asgn.reviewer_status?.replace("_", " ")}
                                </span>
                              </div>
                              <div className="mt-2">
                                {hasMessages ? (
                                  <div className="flex flex-col gap-2">
                                    {asgn.messages.map((msg, idx) => {
                                      ...
                                      return (
                                        ...
                                        >
                                          {msg?.is_editor_approved === "1" ? (
                                            <p className="text-sm text-gray-800">
                                              {msg?.message_to_author ||
                                                "No message provided."}
                                            </p>
                                          ) : (
