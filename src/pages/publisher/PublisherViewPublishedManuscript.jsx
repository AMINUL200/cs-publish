...
const PublisherViewPublishedManuscriptList = () => {
  ...
  useEffect(() => {
    if (!selectedJournal || selectedJournal === "all") return;
  ...
  useEffect(() => {
    if (selectedJournal === "all") {
      ...
      setFilteredManuscripts(
        manuscripts.filter(
          (manuscript) => manuscript.j_title === selectedJournal,
        ),
      );
    }
  ...
  const fetchVolumes = async () => {
    try {
      ...
      console.log("Fetching volumes for journal ID:", journalId);
      // const response = await axios.get(`${API_URL}api/volume/list`, {
      const response = await axios.get(
        `${API_URL}api/journals/${journalId}/volumes`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const journalTitles = [
    ...new Set(manuscripts.map((manuscript) => manuscript.j_title)),
  ];
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      setSelectedManuscripts(
        filteredManuscripts.map((manuscript) => manuscript.id),
      );
    }
  ...
  useEffect(() => {
    if (
      ...
      selectedManuscripts.length === filteredManuscripts.length
    ) {
      setSelectAll(true);
    } else {
      setSelectAll(false);
  ...
  const handleOpenVolumeModal = () => {
    if (!selectedJournal || selectedJournal === "all") {
      toast.warning("Please select a journal first");
      return;
    }
  ...
  const handlePushToArchive = async () => {
    ...
    if (
      window.confirm(
        `Are you sure you want to archive ${selectedManuscripts.length} manuscript(s) to volume ${selectedVolume}?`,
      )
    ) {
      try {
        ...
        if (response.data.status) {
          ...
          setManuscripts(
            manuscripts.filter((ms) => !selectedManuscripts.includes(ms.id)),
          );
          setSelectedManuscripts([]);
          setSelectAll(false);
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            {selectedManuscripts.length > 0 && (
              <button
                ...
                disabled={!selectedJournal || selectedJournal === "all"}
                className={`px-6 py-3 rounded-lg text-white flex items-center justify-center text-sm font-medium transition-all duration-300 shadow-lg
      ${
            ...
            <div className="p-6">
              ...
              <div className="mb-6">
                <p className="text-gray-600 mb-4">
                  Please select a volume to archive {selectedManuscripts.length}{" "}
                  manuscript(s) to:
                </p>
                  ...
                  >
                    ...
                    {volumes.map((volume) => (
                      <option key={volume.id} value={volume.id}>
                        Volume {volume.volume} - {volume.issue_no} (
                        {volume.page_no})
                      </option>
                    ))}
                  </select>
