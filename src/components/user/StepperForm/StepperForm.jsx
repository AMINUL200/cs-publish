...
const StepperForm = () => {
    ...
    const fetchManuscriptForUpdate = async (manuscriptId) => {
        try {
            const response = await axios.get(`${API_URL}api/author/manuscript-edit/${manuscriptId}`, {
                headers: {
            ...
            if (response.data.flag === 1) {
                ...
                setFormData({
                    ...
                    add_myself: manuscript.add_myself === "1" || manuscript.add_myself === 1 ? true : false,
                    ...
                    conflict_of_interest_statement: manuscript.conflict_of_interest_statement || "",
                    references: manuscript.references || "",
                    keywords: Array.isArray(manuscript.keywords) ? manuscript.keywords : [],
                ...
                if (Array.isArray(manuscript.author) && manuscript.author.length > 0) {
                    setAuthors(manuscript.author.map(aut => ({
                        name: aut.name || "",
                        email: aut.email || "",
    ...
    const nextStep = () => {
        ...
        if (step === 1) {
            ...
            const hasValidAuthor = authors.some(author => author.name && author.email);
            if (!hasValidAuthor) {
    ...
    const validateForm = () => {
        const requiredFields = [
            'journal_id', 'username', 'contact_number', 'email',
            'affiliation', 'address', 'country', 'title'
        ];
    ...
    const handleGeneratePDF = () => {
        ...
        if (!formData.title || !formData.journal_id) {
            toast.error("Please fill in at least the title and select a journal to generate PDF");
            return;
        }

        const validAuthors = authors.filter(author => author.name && author.email);
        if (validAuthors.length === 0) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (isUpdateMode) {
                ...
                formDataToSend.append('materials_and_methods', formData.materials_and_methods);
                ...
                formDataToSend.append('author_contributions', formData.author_contributions);
                formDataToSend.append('conflict_of_interest_statement', formData.conflict_of_interest_statement);
                ...
                validAuthors.forEach((author, index) => {
                    ...
                    formDataToSend.append(`author[${index}][university]`, author.university || "");
                    formDataToSend.append(`author[${index}][affiliation]`, author.affiliation || "");
                });
                ...
                if (formData.supplementary_files instanceof File) {
                    formDataToSend.append('supplementary_files', formData.supplementary_files);
                }

                // Use POST method with _method override for Laravel
                formDataToSend.append('_method', 'PUT');

                const response = await axios.post(`${API_URL}api/author/manuscript-update/${updateId}`, formDataToSend, {
                    headers: {
                        'Authorization': `Bearer ${token}`,
                ...
                if (response.data.flag === 1) {
                    toast.success(response.data.message || "Manuscript updated successfully");
                    ...
                    try {
                        ...
                        console.error("PDF generation error:", error);
                        toast.error("PDF generation failed, but manuscript was updated successfully");
                    }
                } else {
            ...
            toast.error(isUpdateMode ? "Update failed. Check console for details." : "Submission failed. Check console for details.");
        ...
        <div className="w-full mx-auto p-6 bg-white shadow-md rounded">
            ...
            <div className="mb-4">
                <h1 className="text-2xl font-bold text-gray-800">
                    {isUpdateMode ? `Update Manuscript (ID: ${updateId})` : 'Submit New Manuscript'}
                </h1>
                {isUpdateMode && (
                    <p className="text-sm text-blue-600 mt-1">
                        You are editing an existing manuscript. All changes will be saved as an update.
                    </p>
                )}
            </div>
            ...
            <div className="mb-6 flex justify-between">
                {["Article Info", "Manuscript", "Files"].map((label, idx) => (
                    <div key={idx} className="flex flex-col items-center">
                        <div
                            className={`w-8 h-8 flex items-center justify-center rounded-full text-white font-semibold ${step === idx + 1 ? "bg-blue-500" : step > idx + 1 ? "bg-green-500" : "bg-gray-300"
                                }`}
                        >
                            {step > idx + 1 ? "✓" : idx + 1}
                        </div>
            ...
            <div className="mt-6 flex justify-between items-center">
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        title={!formData.title || !formData.journal_id ? "Fill in title and select journal to generate PDF" : "Generate PDF from current form data"}
                    >
                        📄 Generate PDF
                    </button>
                </div>
                ...
                {step < 3 ? (
                    ...
                    <button
                        ...
                        className={`px-6 py-2 text-white rounded ${submitLoading
                    ...
                    >
                        {submitLoading
                            ...
                            : (isUpdateMode ? "Update Manuscript" : "Submit Manuscript")
                        }
                    </button>
                )}
            </div>
