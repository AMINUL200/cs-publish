...
const PublicationCharge = () => {
  ...
  // Fetch publication charges data
  const fetchPublicationCharges = async () => {
    try {
      ...
      if (response.data.status) {
        ...
        setPublicationCharges(sortedData);
        setFilteredCharges(sortedData);
  ...
  const handleJournalSelect = (journalId) => {
    const selected = publicationCharges.find(item => item.journal_id === journalId);
  ...
  const handleSearch = (searchTerm) => {
    if (!searchTerm.trim()) {
      setFilteredCharges(publicationCharges);
      return;
    }
    
    const filtered = publicationCharges.filter(item =>
    ...
    return (
      <div className="min-h-screen bg-amber-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-amber-800 text-lg">Loading publication charges...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-amber-50 py-12 sm:py-26">
      ...
      <div className="bg-yellow-600 text-white">
        <div className="container mx-auto px-4 py-12">
          <div className="text-center max-w-4xl mx-auto">
            <h1 className="text-4xl md:text-5xl font-bold mb-4">Publication Charges</h1>
            <p className="text-amber-100 text-lg md:text-xl">
              Transparent and competitive publication charges for all our journals
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
                        ...
                        <div className="flex items-start space-x-4">
                          ...
                          <div className="flex-1">
                            ...
                            <p className={`text-sm ${
                              ...
                              Click to view publication charges
                            </p>
                          </div>
                          {selectedJournal?.journal_id === item.journal_id && (
          ...
          {/* Right Content - Publication Charge Details */}
          <div className="lg:w-2/3">
            {selectedJournal ? (
                  ...
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                    <div>
                      ...
                      <p className="text-amber-100 opacity-90">
                        Publication Charges & Fee Structure
                      </p>
                    </div>
                    {/* <div className="mt-4 md:mt-0">
                      <span className="inline-block bg-amber-700 text-amber-100 px-4 py-2 rounded-full text-sm font-semibold">
                        Journal ID: #{selectedJournal.journal.id}
                      </span>
                    </div> */}
                  </div>
                </div>
              ...
              <div className="bg-white rounded-2xl shadow-lg border border-amber-200 p-12 text-center">
                ...
                <p className="text-gray-600 mb-4">Please select a journal from the list to view its publication charges.</p>
                <button 
                  onClick={() => filteredCharges.length > 0 && handleJournalSelect(filteredCharges[0].journal_id)}
