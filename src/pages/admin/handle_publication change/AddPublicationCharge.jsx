    ...
    try {
      ...
      if (response.data.status && response.data.data) {
        ...
        setFormData({
          ...
          pages: data.pages || "publication_charges"
        });
      }
    } catch (err) {
      toast.error("Error fetching publication charge data: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading && isEdit) {
    return (
      <div className="flex justify-center items-center min-h-64">
        <div className="text-lg">Loading publication charge data...</div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <h1 className="text-3xl font-bold text-gray-800 mb-2">
        {isEdit ? 'Edit Publication Charge' : 'Add New Publication Charge'}
      </h1>
      
      <p className="text-gray-600 mb-6">
        {isEdit ? 'Update the publication charge information' : 'Create a new publication charge for a journal'}
      </p>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              disabled={loading}
              className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200"
              placeholder="Enter publication charge title"
            />
          </div>

          {/* Pages (Hidden field since it's always publication_charges) */}
          <input type="hidden" name="pages" value="publication_charges" />
          ...
          <div className="flex space-x-4 pt-6 border-t border-gray-200">
            ...
            >
              {loading ? (
                ...
                isEdit ? 'Update Publication Charge' : 'Create Publication Charge'
              )}
            </button>
            <button
              type="button"
              onClick={() => navigate('/handle-publication-charge')}
              disabled={loading}
...
export default AddPublicationCharge;